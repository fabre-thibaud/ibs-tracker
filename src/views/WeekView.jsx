...
export default function WeekView() {
  ...
  // Aggregate
  let meals = [], pain = [], bowel = [], summaries = []
  ...
  const avgPain = pain.length > 0
    ? (pain.reduce((s, p) => s + (p.severity || 0), 0) / pain.length).toFixed(1)
    : null

  const painLocations = {}
  for (const p of pain) {
    if (p.location) painLocations[p.location] = (painLocations[p.location] || 0) + 1
  }

  const bristolDist = {}
  for (const b of bowel) {
    if (b.bristolType) bristolDist[b.bristolType] = (bristolDist[b.bristolType] || 0) + 1
  }

  const avgFeeling = summaries.length > 0
    ? (summaries.reduce((s, d) => s + (d.feeling || 0), 0) / summaries.length).toFixed(1)
    : null
  const avgEnergy = summaries.length > 0
    ? (summaries.reduce((s, d) => s + (d.energy || 0), 0) / summaries.length).toFixed(1)
    : null
  const sleepSummaries = summaries.filter((d) => d.sleep)
  const avgSleep = sleepSummaries.length > 0
    ? (sleepSummaries.reduce((s, d) => s + d.sleep, 0) / sleepSummaries.length).toFixed(1)
    : null

  const hasData = meals.length > 0 || pain.length > 0 || bowel.length > 0 || summaries.length > 0
  ...
  return (
    <div className="week-view">
      <div className="week-nav">
        <button className="week-nav-btn" onClick={prevWeek} aria-label="Previous week">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <span className="week-nav-title">{weekRange}</span>
        <button className="week-nav-btn" onClick={nextWeek} aria-label="Next week">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
        </button>
      </div>

      {!hasData && (
        <div className="week-empty">No data for this week.</div>
      )}
                ...
                <div style={{ marginTop: 8 }}>
                  <div className="week-list">
                    {Object.entries(painLocations).map(([loc, count]) => (
                      <div key={loc} className="week-list-item">{loc} ({count}x)</div>
                    ))}
                  </div>
                </div>
              )}
              ...
              {Object.keys(bristolDist).length > 0 && (
                <div className="week-list" style={{ marginTop: 8 }}>
                  {Object.entries(bristolDist)
                    ...
                    .map(([type, count]) => (
                      <div key={type} className="week-list-item">Type {type}: {count} times</div>
                    ))}
                </div>
              )}
            </div>
