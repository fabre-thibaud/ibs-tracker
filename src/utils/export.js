import { getWeekDays, formatWeekRange, fromDateKey } from './dates.js'
...
export function generateWeeklySummary(data, dateKey) {
  ...
  // Pain
  const avgPain = allPain.length > 0
    ? (allPain.reduce((s, p) => s + (p.severity || 0), 0) / allPain.length).toFixed(1)
    : 0
  const painLocations = {}
  ...
  const avgEnergy = summaries.length > 0
    ...
    : 'N/A'
  const avgSleep = summaries.filter((d) => d.sleep).length > 0
    ? (summaries.reduce((s, d) => s + (d.sleep || 0), 0) / summaries.filter((d) => d.sleep).length).toFixed(1)
    : 'N/A'
  ...
  if (Object.keys(bristolDist).length > 0) {
    ...
    for (const [type, count] of Object.entries(bristolDist).sort((a, b) => Number(a[0]) - Number(b[0]))) {
      lines.push(`  * Type ${type}: ${count} times`)
    }
  }
  if (Object.keys(colorNotes).length > 0) {
    const colorStr = Object.entries(colorNotes).map(([c, n]) => `${n}x ${c.toLowerCase()}`).join(', ')
